...
- **Project registry:** `projects.json` in the hub repo. Each entry has `slug`, `name`, `tagline`, `type` (`app` or `showcase`), `status` (`live`, `demo`, `coming`, `archived`), `url`, `repo`, `healthUrl`, `tags`, `hero`. The hub renders cards from it; adding a project is one JSON entry plus a push. Showcase entries (UnPark/Antifreeze, RC car, Porsche PCB keychain, DIY EEG, FlashCards; Outline and classmyschedule were removed 2026-09-18) have no `url` or `healthUrl` and link to `/projects/<slug>` on the hub.
