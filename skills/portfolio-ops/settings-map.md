...
| Vercel CLI login | every Vercel operation (`npx vercel ...`) | `~/.vercel` auth on Kalp's Mac (already logged in, team `kks-projects-2edcb11a`) | Kalp (`npx vercel login`) | no deploys, env changes, or domain changes from the CLI; Git-integration deploys still happen | works now |
