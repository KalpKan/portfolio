...
| Deploy a browser-ML app (MediaPipe) to Vercel | Written and executed 2026-09-18 (T3.1, pushups: `KalpKan/pushup-tracker-web` → project `pushups`, `https://pushups.kalpkan.com`); FIX r2 2026-09-19 added the versioned-model-URL rule, classifier augmentation and the hints-never-cost-a-rep rule; emotes FIX r3 2026-09-19 added step 12 (two models settle at different speeds: wait while the slow one is undecided, gate at the real frame rate); emotes TEST r4 2026-09-20 added step 14 (vary the rest frames, re-test a conflict fix on the far variant, run the whole positive set live) |
...
- **Train the classifier on the landmarks of the model you ship.** Landmarks differ between MediaPipe generations even for the "same" network: the Tasks `pose_landmarker_full.task` puts the wrist/elbow `z` up to 0.43 off the legacy `mp.solutions.pose` (`model_complexity=1`), and lite is worse still. The pushups classifier trained on legacy landmarks called every bottom of a bad clip "good" in the browser while agreeing with Python on a good clip (incidents 2026-09-18 lite-vs-full and 2026-09-19 D3). Since FIX r1 the recipe is: extract landmarks from the training clips with the shipped `.task` file in Python (`scripts/make_training_landmarks.py`, the Tasks API is in the same `mediapipe` wheel), train on x, y only (hip-centred, torso-scaled, aspect-corrected, mirrored to one facing; `scripts/train_form_model.py`), hold out by clip, convert with `tensorflowjs_converter`, and pin TF.js to Keras within 1e-4 on a held-out fixture. Then judge the classifier on people it never saw (the corpus's external clips) before trusting it outside the training orientation; the pushups one did not transfer, so it is gated.
- **Bake the preprocessing.** The trainer writes the standardisation mean/scale straight into a generated `src/scaler.ts` next to the held-out accuracy, so the README can quote real numbers and the page and the tests share one set of constants.
- **New model bytes = new URL (FIX r2, 2026-09-19).** `/models/*` and `/wasm/*` are served `immutable` for a year, so a retrained classifier written over the same filenames reaches nobody who has visited before: the 36 → 24-input retrain of `c446bcb` gave every returning browser 254 silent exceptions per demo play and "0 good of 0" (incidents 2026-09-19 D1). Rule: the model directory carries a version (`public/models/form-v3/`, `MODEL_URL` in `src/classifier.ts`), the old directory is deleted (so a stale URL 404s instead of serving old bytes), a test pins the versioned path (`tests/classifierUrl.test.ts`), the loader checks `model.inputs[0].shape` against the feature width and retries once with `requestInit: {cache: "reload"}`, and the frame loop is wrapped in try/catch so any exception stops the session with a status message (`onError` → "Something went wrong: … Reload the page and try again." + a `session_failed` event). `scripts/e2e-failure-modes.mjs <url> <dir with the v1 model files> <out>` proves all three paths in headless Chrome. The same rule applies to a `@mediapipe/tasks-vision` bump (`/wasm/*` filenames) and to the `.task` file.
- **Make the classifier robust to the browser's landmarks (FIX r2, 2026-09-19).** Even with the same `.task` file, landmarks from the GPU delegate on MJPEG camera frames sit a few pixels away from the Python extraction, and a small MLP trained without augmentation flips 0.99 ↔ 0.03 on consecutive frames of one rep (classifier alone at the labelled bottoms: 53/69 Python vs 45/69 browser). `scripts/train_form_model.py` v3 feeds every training frame through 6 random perturbations of the raw landmarks (Gaussian jitter σ = 0.012 frame heights, ±4° rotation about the hips, ±10 % scale, a blend with a neighbouring frame) and averages 5 seeds inside one Keras model (`Average` layer; one 27 KB TF.js file); `scripts/eval_form_model.py [model.h5] [scaler.json]` scores any candidate against the labelled bottoms on BOTH trace sets in seconds, which is how the settings were chosen (held-out accuracy first, the corpus second; do not tune on the corpus alone, it is the test set). Ask a per-frame classifier over the phase it was trained on: v2 was trained on the lower half of each clip but consulted at the extreme bottom only, where a body collapsed on the floor looks straight and a clean chest-to-floor rep looks like a sag; averaging it over the bottom window plus the whole ascent (where the chest-first "worm" and the collapse show) took the corpus from 62/69 and 57/69 to 65/67 on both sets. Judge the top of a rep on the 0.4 s before the descent, not on everything since the last rep (a rest with the hips down between reps otherwise condemns the next one). A time-based EMA on the classifier input was tried and gave nothing.
- **Hints must never cost a rep (FIX r2, 2026-09-19).** Pausing the counter under every placement hint silently zeroed `test_video` and lost a rep of `good_IMG_4378`: Kalp's own clips dip the nose to x = 1.03 at every clean bottom. Split severity: the hint shows for any landmark at the edge; counting pauses (grey skeleton, "Counting paused") only for no pose, darkness, two people, a frontal view, or a head / both feet more than 6 % outside the frame or invisible (`pausesCounting` in `src/hints.ts`). Hold hints with hysteresis on "any problem present", not per text (a half-detected body flickers between "no pose" and "head out"). A second pose is another person only when its torso is ≥ 0.4× the first's and their boxes overlap < 0.5 (MediaPipe returns a 0.15–0.5× phantom inside the box of a body lying flat). `?trace=full` + `DUMP_DIR=… scripts/e2e-hints.mjs` dumps every pose with visibility and the raw/shown hint per frame, which is how those numbers were read off; MediaPipe's `visibility` stays ≈ 1 even for a guessed head, so use positions, not visibility, for "out of frame". After any change to hints re-run `e2e-corpus.mjs`: the corpus must not move.
...
- **PostHog:** never send landmark or image data. Type the `capture()` signature so only `session_started {mode}`, `rep_counted {good}` / `emote_fired {...}`, `demo_video_played`, `session_failed {mode, message}` compile.
...
3. For an exact check, record the page's own per-frame landmarks (`?trace` in the URL fills `window.__pushupsTrace`; `TRACE_DIR=… scripts/e2e-corpus.mjs` saves one JSON per clip) and commit them as a second corpus set (`tests/fixtures/traces-browser/`) that the unit gate replays through the real pipeline; a change that only works on Python landmarks then fails `npm test` (FIX r1, 2026-09-19).
...
11. **Believe a corpus number only after two more checks (emotes TEST r2, 2026-09-19).** (a) *Held-out transforms:* mirror, letterbox into a 480 × 640 portrait frame at 78 %, and shrink to 60 % every corpus photo (`docs/reports/evidence/emotes-r2-make-variants.py`), re-extract with `scripts/extract_still_landmarks.py --src <variant dir> --out <scratch dir>` (copy `labels.json` in first so the kinds carry over) and score with `emotes-r2-variant-eval.ts`; precision must hold at 100 % and recall is the honest number (the 95 originals are the training set). (b) *VIDEO-mode landmarks:* the page runs the models in VIDEO mode, the corpus is IMAGE mode; `emotes-r2-video-landmarks.py <clip.mjpeg> <out.json> [step]` dumps per-frame landmarks from a fake-camera clip with the same `.task` files, and `emotes-r2-video-eval.ts` replays them through the engine + gate with the cue breakdown per frame. Any per-frame conflict rule (one gesture vetoing another) must be tested this way: the lite pose model's wrist height on a static frame jitters a whole 0.2-shoulder-width band, which turned `thumbs_up-04` into a Goblin Muscle 8 times in 13 while the corpus said 1.00. (c) Sequence timing: the gate's cooldown and the engine's one-shot edge interact; a clip with < 1 s between gestures belongs in the corpus.
12. **Two models, two clocks (emotes FIX r3, 2026-09-19).** When a rule fuses landmarks from separate MediaPipe models, remember they settle at different speeds: the hand landmarker reports a fist on the first frame, the lite pose landmarker in VIDEO mode needs 300–400 ms after a fast move (or a hard cut) to place the arm, and MediaPipe hands back a stale or half-landed pose meanwhile, not "no pose". A per-frame conflict rule between the two (thumbs-up vs flex) therefore decides on stale evidence, and even a one-frame head start beats a 150 ms charge while a 0.2 s average is still climbing. Make the decision wait while the slow model is *undecided*, measured with cheap cross-model checks: a hand whose wrist no pose wrist is within 0.3 shoulder widths of (stale), a pose wrist that jumped > 0.3 shoulder widths since the last frame (moving), a raw score still > 0.2 above its own average (rising); require the decision to hold for a settle time longer than the two-frame plateau a landing pose shows (200 ms at 20 fps), cap the wait (500 ms) so a pose that never agrees fires anyway, and restart the slow model's average when a stale pose lands. Do not restart it on a raw "step" alone: at 20 fps a jittery half-flex reads ≥ 0.9 on consecutive frames often enough to flip the decision (it fired Goblin Muscle 7 of 8 passes live while the 10 fps gate stayed green). Sample at least one VIDEO-mode fixture at the page's real frame rate (`extract_video_landmarks.py --step 1`), and add a `?trace` per-frame buffer to the page (`TRACE=… npm run e2e`) before tuning: the missed passes here were a ~7 % timing window that only the real pipeline showed, and the page's `?trace` shifted the frame phase enough to hide them, so read the trace for the mechanism and count the untraced runs for the rate.
12. **VIDEO-mode reels as a committed gate (emotes FIX r2, 2026-09-19).** Turn check 11(b) into fixtures: `python3 scripts/build_e2e_clips.py <dir> all` (named slideshows from the read-only photo folder, 30 fps `.mjpeg` + labels, never committed) → `/usr/bin/python3 scripts/extract_video_landmarks.py <dir>/e2e-<name>.mjpeg <dir>/e2e-<name>-labels.json <dir>/<name>.json --step 3` (the site's own `.task` models in `RunningMode.VIDEO`, every third frame = 10 fps; run it in the foreground or with `run_in_background`, a `&` inside a tool call dies with the shell) → `npx vite-node scripts/compact-video-fixture.ts <dir>/<name>.json tests/fixtures/video/<name>.json` (pose + hands kept, the 478-point face reduced to `faceMetrics`, 4 decimals: 9 reels = 1.7 MB instead of 12). `tests/video.test.ts` replays them through the same `runInputs` the clip scripts use and is in `npm run test:corpus` and CI. Design rules that came out of it: smooth every raw score with a time-constant EMA (`1 - exp(-dt/tau)`, tau ≈ 200 ms) BEFORE any conflict rule; compare conflicting gestures by ratio, not difference, so the climbing first frames of a hold are judged like a settled one; let the emote gate run every frame with a pending edge instead of consuming a one-frame edge inside a cooldown; hold UI hints by time. Expect the corpus latency median to rise by about the EMA's time-to-0.5 (≈ 80 ms here) and the worst case to fall. A hard cut in a slideshow makes the hand model settle a frame before the pose model (a flexing fist with the thumb up plays Thumbs Up on video, flex on the steady photo): give such an event `accept: [gesture]` in the labels rather than tuning for a cut no real arm makes, and keep the invariant that one hold plays one emote.
13. **What a fake-camera reel cannot excuse, and how to stress the pipeline without a phone (emotes TEST r3, 2026-09-19).** An `accept: [other gesture]` on a *clear positive* hides a wrong emote from the gate: `flex09x3` printed PASS while playing Thumbs Up for a flex 5 of 6 times live; a reviewer should treat any `accept` that names a different gesture as a defect unless the photo itself is ambiguous. For the stress tests, `docs/reports/evidence/emotes-r3-e2e-harness.mjs` is the repo judge plus `THROTTLE=n` (CDP `Emulation.setCPUThrottlingRate`, 4 ≈ 14 fps and 6 ≈ 10–13 fps on this Mac, the honest stand-in for a phone), a pipeline fps counter (MutationObserver on a meter value; print it on every run so a stalled fake camera is distinguishable from a missed gesture), stream / stage geometry (a portrait `.mjpeg` comes out of Chrome's fake device as 480 × 480 because the site asks for 640 × 480, which still exercises the stage-aspect code) and `BROWSER=playwright` (Playwright's Chromium from `~/projects/promptflip/node_modules`, read only, same flags). `emotes-r3-build-e2e-clips.py` adds slow zoom-and-sway motion, a dim blurry variant (brightness 0.45, blur 1.5, JPEG 55), 0.6 s rests and a 30 s neutral mix; keep `run_in_background` batches to one Chrome per GPU and expect ~1.5 min per live run (each fresh profile downloads the 39 MB of models), ~1 min locally. Offline probes that need no photos: talking with all five lip pairs and a synthetic laugh on the real neutral faces (`emotes-r3-talk-laugh-synth`), and gestures morphing into one another with the corpus synthesiser's transitions (`emotes-r3-morph-synth`).

14. **Vary the rest, not only the gesture; test the conflict fix where it hurts (emotes TEST r4, 2026-09-20).** A tracking model (MediaPipe pose in VIDEO mode) gives a *different* answer for the same frame depending on what it tracked before: two clear flexes fired 4/4 after a no-hands rest and 1/4 – 0/4 after a rest of a person with hands on a desk, with the tracker alternating no-pose / wrong-pose for the whole 2.5 s hold, and a crossfade instead of a hard cut changed nothing. So every reel set must vary the frames *between* gestures (no hands, hands on a desk, hands at the sides; `emotes-r4-build-e2e-clips.py` alternates them in `all-*` and `other-rest`), the page needs a re-seed path for a tracker that alternates present / absent (an IMAGE-mode `detect()` on the current frame, or take the wrist from the hand model), and "recall 100 %" on reels that all start from one rest frame is not evidence. Second rule: re-run a conflict-rule fix on the far (55 %) and mirrored variants of the *photo that motivated it* before calling it fixed: the round-3 wait rule holds at full scale (3/3 ×5) and expires into the wrong emote at 55 % (Thumbs Up 3/15) because its 500 ms cap fires the contested gesture; a cap should fire nothing and re-arm. Third: any rule that suppresses an edge (the one-fist takeover) gets a reel where the suppressed gesture is the intended one after a long hold (`takeover`: 4 of 6 second emotes lost). Fourth: run the whole clear positive set once through the live pipeline at both widths (`all-flex`, `all-thumbs`, `all-yawn`, ~10 min per width, one Chrome at a time) and quote *that* recall in the report, since the corpus gate is 100 % and says nothing new; keep `run_in_background` batches under the tool's 10 min limit or launch them with `nohup bash -c '…' &` and poll the output file.
...
## Secret scan (run before any repo goes public, and after agent bursts)

`scripts/secret-scan.sh` in the hub repo walks every git repo under `~/projects`, greps the full history of all branches for the exact operator secret values in `~/.config/portfolio-ops/secrets.env` (never printed) and for key-shaped patterns (JWTs, sk-, phx_, napi_, cfut_, sbp_, Postgres URLs with passwords, private keys, UptimeRobot and Pl@ntNet keys). `EXACT:` means rotate that key immediately and purge history; `pattern:` needs a human look because test fixtures match too. Also confirms no `.env` file is tracked. Last full run 2026-09-19: no exact hits anywhere; one real historical leak in Plato (see incidents 2026-09-19).
