...
| Add a case study | Written and executed 2026-09-18 (T4.1: unpark, rc-car, yash-birthday-pcb live; eeg published as "under construction" with status `coming` (H14); flashcards reduced to the placeholder page and Outline removed per Kalp 17:57; classmyschedule removed 18:05, H9 closed; reviewer fixes 2026-09-18: stacked edge labels, DRC wording, draft gate documented) |
...
**As executed 2026-09-18 (T4.1):** live: `/projects/unpark`, `/projects/rc-car`, `/projects/yash-birthday-pcb` (KiCad renders); under construction: `/projects/eeg`; placeholder page: `/projects/flashcards` (draft content). Outline was removed at 17:57 on Kalp's instruction (not a real project) and classmyschedule was removed from the registry at 18:05 (not Kalp's work, H9 closed), so `/projects/classmyschedule` is a 404. RC-car photos are frames from `~/Desktop/Out and About/Sidequest/Automatic-RC-Car/Videos/*.mp4` (never committed). The Chrome window would not resize below 1440 px (`innerWidth` stayed 1440 after `resize_window`), so the 390 px check was done with a same-origin `<iframe>` of the page at 390×640 injected into the desktop tab, which honours media queries and reports its own `scrollWidth`.
...
- `vercel --prod` (or the GitHub push) answers `Resource is limited - try again in 24 hours (code: "api-deployments-free-per-day")` and the commit's GitHub status says `Deployment rate limited — retry in 24 hours`: the **team** has made 100 deployments in 24 h (Hobby limit, all projects together; the hub alone made 64 on 2026-09-18). Nothing free unblocks it. Vercel does **not** retry the Git deploy later: once the window has moved (`GET /v6/deployments?teamId=...&since=<now-24h>` well under 100), push any commit or run `npx vercel@latest --prod --yes --scope kks-projects-2edcb11a` from the repo root. Until then production serves the previous commit; say so in STATUS.md. Check the budget first: `~/projects/portfolio/scripts/vercel-deploy-budget.sh` prints the team's deployments in the last 24 h by project and when a slot frees (2026-09-18 22:55 UTC: 99/100, 65 from the hub, oldest ages out 2026-09-19 18:55 UTC). To deploy as soon as the cap lifts without babysitting a Terminal, run `~/projects/portfolio/scripts/vercel-redeploy-when-quota-frees.sh ~/projects/<app> <app>.kalpkan.com 32 15` (retries every 15 min for 8 h, then checks `/api/health`; log in `~/.config/portfolio-ops/logs/`). It must be started by a person or an agent that is alive at the time: installing it as a launchd job was refused by the agent permission layer ("unauthorized persistence"), so no agent can schedule it. Prevention: batch commits before pushing (one deploy per task step, not per file), and never loop `vercel --prod` to "refresh" a build. **An "Ignored Build Step" on the hub does not help**: Vercel's docs state that canceled builds "will still count towards your deployment quotas", so docs-only pushes still burn a slot; the only $0 way to stop the hub eating the budget is fewer pushes (agents: commit STATUS.md/skills/docs once per task, not per step), or turning the hub's auto-deploy off in `vercel.json` (`git.deploymentEnabled: {"main": false}`) and deploying it by hand with `vercel --prod`, which is a policy change for the hub owner, not something a project fixer should flip.
