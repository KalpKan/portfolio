...
| `hoops.` | **https://hoops.kalpkan.com** (live since 2026-09-18; CNAME `71da701c9c3d8bdf.vercel-dns-017.com`, DNS-only; fallback alias `https://v0-shootersshoot.vercel.app`). No `/api/health` route yet. | Vercel Hobby, project `v0-basketball-analytics-dashboard` (its latest production build had errored since 2026-04-15, which blocks `domains add`; fixed 2026-09-18 by `vercel redeploy` of the last Ready build, see `docs/DNS_PENDING.md` §5) | `KalpKan/Basketball-Stat-Tracker` (public) | Supabase **Project B** `yzppfufqaekgaxcrsqxp` (to be renamed `platform`, schema `hoops`) in T1.1 | Vercel project Environment Variables (currently unset, so the dashboard silently shows mock data) | domain live; app wiring PENDING (Phase 1, T1.1) |
