...
| Showcase pages (`/projects/<slug>` on the hub) | PENDING (placeholder pages exist; a showcase row links to its repo and says "case study soon" while its registry `status` is `coming`, and switches to "read" → `/projects/<slug>` once set to `live`; runbook "Add a project", kind table) | hub, rendered from `projects.json` entries with `type: "showcase"` | `KalpKan/portfolio` | none | none | PENDING (Phase 4) |
