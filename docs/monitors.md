...
|---|---|---|---|
| Kalp's projects | **https://stats.uptimerobot.com/a6n3Wx3PBp** | the six monitors below | `1263036` |
...
| `hoops dashboard` | https://hoops.kalpkan.com/ (moved from `https://v0-basketball-analytics-dashboard-seven.vercel.app/` on 2026-09-18 22:40Z via `PATCH /v3/monitors/804030256`, same id) | 5 min | basketball dashboard on Vercel (liveness; since T1.1 the page reads Project B, but the keep-alive is the dedicated monitor below) | No (see `platform health (DB, Project B)`) | `804030256` |
...
- **After H1 (custom domain):** done. All three original monitors were re-pointed with `PATCH /v3/monitors/{id}` (promptflip during T0.4, hoops and hub during the Phase 1 audit on 2026-09-18), keeping their ids and uptime history. No monitor targets a `*.vercel.app` host any more; pushups, emotes and microtubules (static, no database) have no monitor by design, their `health.json` is polled by the hub's live marks instead.
- **Pre-existing monitors, not part of the platform:** the account also holds six paused keyword monitors from 2020 (ids `784431015`, `784431036`, `784431038`, `784431041`, `784431055`, `784431056`, all on `shop.travisscott.com`). They are Kalp's, were left untouched, are not on the status page, and count toward the 50-monitor limit (12 of 50 used).
