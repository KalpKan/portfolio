...
- §3 follow-ups, all done: the hoops monitor (`804030256`) moved to `https://hoops.kalpkan.com/` on 2026-09-18 22:40Z (Phase 1 audit) and the hub monitor with it; `hoops` still has no `/api/health` route (`https://hoops.kalpkan.com/api/health` → 404, a T5.a defect), so its `projects.json` `healthUrl` is Project B's `https://yzppfufqaekgaxcrsqxp.supabase.co/functions/v1/health` (the database behind the dashboard) rather than `null`. Nine monitors exist since 2026-09-19 (`docs/monitors.md`).
