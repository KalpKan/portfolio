...
- Hub KalpOS menu (T6.2, 2026-09-20): `menu_action { item }` with `item` one of `about`, `lock`, `restart`, `mute`: a row chosen in the KalpOS menu (menubar brand word, or ■ KalpOS on a phone), or its keyboard shortcut (⌘L → `lock`, ⌃⌘R → `restart`). `restart` is sent when the row is chosen, before the confirm sheet, so a cancelled restart still counts as one `restart`; a completed restart is followed by a second `chime_played { at }` and `unlocked`, since the boot replays. Nothing carries which chime state was toggled.
