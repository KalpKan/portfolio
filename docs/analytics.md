...
- Hub desk (T6 KalpOS, 2026-09-20): `unlocked` (the lock screen was passed; returning visitors skip it and send nothing), `window_opened { slug }` (`projects`, `about`, `contact`, `hobbies`, `trash`, `music`, `terminal` or `case:<slug>`; the phone sheets send the same names), and `project_card_clicked { slug, type, kind }` is unchanged: a live tile (new tab) or a case-study tile (opens a window) in the Projects window or the phone sheet.
