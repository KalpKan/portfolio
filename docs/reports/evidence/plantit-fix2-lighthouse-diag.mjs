...
for (const f of ['monstera-deliciosa.jpg', 'ficus-lyrata.jpg', 'aloe-vera.jpg']) { const fd = new FormData(); fd.append('image', fs.createReadStream(`${FIX}/${f}`), { filename: f, contentType: 'image/jpeg' }); const r2 = await api.post('/api/identify', fd, { headers: fd.getHeaders() }); console.log('seed', f, r2.status, r2.data?.savedPlant?.species); }
...
await page.evaluate(async ({ uid, apiKey, idToken, refreshToken, expiresIn }) => {
  ...
  if (name === 'plants') {
    fs.writeFileSync(process.env.LHR_FILE || 'lhr-plants.json', JSON.stringify(lhr));
    const items = lhr.audits['network-requests'].details.items.map((r) => ({ url: r.url.replace(/^https?:\/\/[^/]+/, '').slice(0, 70), start: Math.round(r.networkRequestTime), end: Math.round(r.networkEndTime), kb: Math.round((r.transferSize || 0) / 1024), pri: r.priority, type: r.resourceType }));
    for (const it of items) console.log(`${String(it.start).padStart(6)} -> ${String(it.end).padStart(6)}  ${String(it.kb).padStart(4)} KB ${String(it.pri||'').padStart(8)} ${it.type||''}  ${it.url}`);
    const ph = lhr.audits['largest-contentful-paint-element']?.details?.items?.[1]?.items || [];
    console.log('LCP phases', JSON.stringify(ph));
    console.log('LCP', lhr.audits['largest-contentful-paint'].numericValue, 'observed', lhr.audits['metrics']?.details?.items?.[0]?.observedLargestContentfulPaint, 'observedLoad', lhr.audits['metrics']?.details?.items?.[0]?.observedLoad);
  }
}
fs.writeFileSync(process.env.OUT_FILE || 'out-local/lighthouse.json', JSON.stringify(out, null, 2));
await ctx.close();
const left = await api.get('/api/plants'); for (const p of left.data || []) await api.delete(`/api/plants/${p.id}`); console.log('deleted', (left.data || []).length);
process.exit(0);
