// Plant It round-3 audit (derived from round 2; adds identify timings, D15/D16 probes, card readings, dialog fold, KEEP mode)
...
const UID = process.env.TEST_UID || 'round3-test';
const KEEP = process.env.KEEP === '1';
const SKIP_IDENTIFY = process.env.SKIP_IDENTIFY === '1';
...
async function identify(page, file, label, opts = {}) {
  ...
  await page.setInputFiles('[data-testid="file-input"]', file.startsWith('/') ? file : path.join(FIX, file));
  ...
  if (enabled && !opts.noClick) {
    ...
    ms = Date.now() - t0;
    after = { url: page.url(), alerts: await page.locator('.MuiAlert-message').allTextContents(), text: (await page.locator('main, #root').first().innerText()).slice(0, 2500), identifyEnabled: await page.locator('[data-testid="identify"]').isEnabled().catch(() => null), previewStillShown: await page.locator('img[alt="Preview"]').count() };
  }
...
async function run() {
  ...
  for (const vp of [{ name: 'desktop', width: 1440, height: 900, mobile: false }, { name: 'phone', width: 390, height: 844, mobile: true }]) {
    ...
    if (!vp.mobile) {
      ...
      R.identifyLow.runnerUpControls = await page.locator('[data-testid="other-candidates"] button, [data-testid="other-candidates"] a').count();
      ...
      R.tooLarge = await identify(page, '/private/tmp/claude-501/-Users-kalp/6a491b3c-d386-4c83-9fec-6077672a40f8/scratchpad/at-limit-6000x5000.jpg', `${vp.name}-too-large`);
    ...
    if (!vp.mobile) {
      ...
      await ref.set({ ...src, id: ref.id, imageUrl: null, photoPath: null, thumbUrl: null, thumbPath: null, species: 'Spathiphyllum wallisii', commonName: 'Peace lily (dry-out seed)', minVWC: 5, maxVWC: 28, optimalVWC: 20, wateringThreshold: 28, currentVWC: 28, createdAt: ago, lastWatered: ago, deviceConnected: false, demo: false, lowConfidence: false });
    ...
    R.plants.cardReadings = await page.locator('[data-testid="card-reading"]').allTextContents();
    R.plants.chipColours = await page.evaluate(() => [...document.querySelectorAll('[data-testid="plant-card"] .MuiChip-root')].map((c) => ({ label: c.textContent, cls: [...c.classList].filter((k) => /color/i.test(k)).join(' ') })));
    R.plants.bodyBg = await page.evaluate(() => getComputedStyle(document.body).backgroundColor);
    R.plants.imgAttrs = await page.evaluate(() => [...document.querySelectorAll('[data-testid="plant-card"] img')].slice(0, 3).map((i) => ({ loading: i.getAttribute('loading'), fetchpriority: i.getAttribute('fetchpriority'), w: i.getAttribute('width'), h: i.getAttribute('height'), src: i.currentSrc.slice(-24) })));
    ...
    R.water.lastWateredLine = (R.water.sensorText.match(/last watered [^\n]*/) || [null])[0]; R.water.logLine = (R.water.sensorText.match(/Watered \([a-z]+\)[^\n]*/) || [null])[0];
    ...
    if (victim.imageUrl) { R.delete.imageAfter = {}; for (const wait of [0, 5000, 20000, 60000]) { await new Promise((r) => setTimeout(r, wait)); R.delete.imageAfter[`t+${Math.round(wait/1000)}s`] = (await axios.get(victim.imageUrl + '?r3=' + Date.now(), { validateStatus: () => true })).status; if (R.delete.imageAfter[`t+${Math.round(wait/1000)}s`] !== 200) break; } R.delete.imageAfterPlain = (await axios.get(victim.imageUrl, { validateStatus: () => true })).status; }
  ...
  if (!KEEP) for (const p of left.data || []) await api.delete(`/api/plants/${p.id}`);
  results.cleanup = { deleted: KEEP ? 0 : (left.data || []).length, kept: KEEP ? (left.data || []).map((p) => p.id) : [] };
  await browser.close();
