...
const PAGES = (process.env.PAGES || 'login,plants').split(',');
for (const [name, url] of [['login', `${BASE}/login`], ['plants', `${BASE}/plants`]].filter(([n]) => PAGES.includes(n))) {
  if (name === 'plants') {
