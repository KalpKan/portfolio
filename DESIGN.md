...
- **Window bodies** (About, Contact, Trash, Music): 13.5/1.55, h2 600 20 px. **Terminal**: Geist Mono 12/1.7 on the dock tile's `#2b2929 → #201e1d`; output cyan `#62c5ee` for directories, ok and the mark, `#ff90b1` for errors, `.55` for the prompt and dim lines; the caret is a drawn block that blinks at 1.1 s only while the input has focus.
...
- **Chime**: an original G major spread (G3 · D4 · G4 · B4, triangle/sine, ±4 cents), 120 ms attack, 500 ms swell, 1.8 s release, low-pass 2.2 kHz, master 0.3, from the Web Audio API. Once per boot (a Restart is a new boot; locking and unlocking again is not): at the boot mark when the browser already allows audio, otherwise on the unlock gesture, 400 ms later with the blur-out. A speaker toggle in the menubar's right cluster mutes it (`kalpos:mute`). It plays under reduced motion (sound is not motion).
...
- **Reduced motion**: the boot is a 400 ms crossfade to the lock and the unlock a 400 ms crossfade to the desk (Lock Screen and Restart, the menu and the confirm sheet are 400 ms fades too), windows appear and vanish in place, the dock does not magnify, the trace is pre-drawn, dots and pulses are static, the crumple is skipped.
