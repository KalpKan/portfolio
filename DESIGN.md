...
**Key Characteristics:**
- Black is the only dark surface (power screen + boot + lock). The desk has no dark mode.
...
- **Black** (`#000`): the power screen, the boot and the lock screen; nothing else.
...
- **Reduced motion**: the power screen is unchanged (it is the gesture); the boot shows the logo and the full bar statically for 800 ms then a 400 ms crossfade to the lock, and the unlock is a 400 ms crossfade to the desk (Lock Screen and Restart, the menu and the confirm sheet are 400 ms fades too), windows appear and vanish in place, the dock does not magnify, the trace is pre-drawn, dots and pulses are static, the crumple is skipped.
