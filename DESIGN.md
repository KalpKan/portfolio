...
- **Reduced motion**: the power screen is unchanged (it is the gesture); the boot shows the logo and the full bar statically for 800 ms then a 400 ms crossfade to the lock, and the unlock is a 400 ms crossfade to the desk (Lock Screen and Restart, the menu and the confirm sheet are 400 ms fades too), windows appear in place, the dock does not magnify, the trace is pre-drawn, dots and pulses are static (the NOW PLAYING dot included; its progress line still advances, in 250 ms steps, since a position is information, not motion).
