...
| `components/kalpos/KalpOSMenu.tsx`, `RestartSheet.tsx` | The **KalpOS** word in the menubar (■ KalpOS on a phone) is a menu: About, **Lock Screen** (⌃⌘Q, the real macOS lock shortcut; ⌘L stays the browser's address bar) and **Restart…** (⌃⌘R), which replays the whole boot (chime, logo, progress bar, lock; no power screen, since the click was the gesture) without reloading the page. The terminal's `reboot` and `lock` do the same. |
